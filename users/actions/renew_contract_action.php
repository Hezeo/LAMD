...
try {
    ...
    if (!empty($idsToDelete)) {
        ...
        // UPDATE: Set status to 'deleted' instead of deleting row
    ...
    try {
        ...
        // Log error but do not stop the main renew process
    ...
    // --- =======================================================================
    try {
        // We insert the existing rows into tbl_renewed_contracts BEFORE we modify tbl_main.
        // This ensures we save the "Previous" state of the data.

        $sqlHistory = "INSERT INTO tbl_renewed_contracts (
                ...
                if (in_array($fileType, $allowedTypes)) {
                    ...
                    if (move_uploaded_file($tmpName, $destination)) {
                        ...
                        try {
                            ...
                            // Log error but don't stop the renew process
            ...
            if (isset($lot['groups']) && is_array($lot['groups'])) {
                foreach ($lot['groups'] as $group) {
                    ...
                    if ($formGroupId && isset($existingMap[$formGroupId])) {
                        ...
                        $stmt_upd = $pdo->prepare($sql_renew);
                        $stmt_upd->execute([
    ...
    $pdo->commit();
        ...
        // ==========================================
    // INSERT ACTIVITY LOG & NOTIFICATIONS (RENEWED CONTRACT)
    // ==========================================
    try {
        ...
        $logStmt->execute([
            // Message: "Renewed a contract of DELA CRUZ"
            ':activity'           => "Renewed a contract of {$new_contracting_party}",
            ':action'             => "renew",
            ':user_id'            => $_SESSION['user_id'],
            ':admin_id'           => null,
    ...
    $response['success'] = true;
    $response['message'] = "Contract renewed successfully.";
