...
function typeWriter(text, element, delay = 100, callback = null) {
  ...
  function typing() {
    if (i < text.length) {
      ...
      // Animation finished: check if there is a callback function
      if (callback) {
        callback();
      }
    }
...
function setGreeting() {
  ...
  // We pass a 'callback' function that runs AFTER "Good Morning" finishes typing
...
//LOGIN VALIDATION (Keep your existing validation code)
