...
if (!$sourceRow && isset($_GET['id'])) {
    ...
    $sql_id = "SELECT * FROM tbl_main WHERE id = ? LIMIT 1";
    ...
    if ($sourceRow) {
        ...
        $sql_reblock = "SELECT * FROM tbl_main 
...
if (!$sourceRow) {
    ...
    <body>
        <div class="error-card">
            <i class='bx bx-file-blank error-icon'></i>
            <h3 class="error-title">Record Not Found</h3>
            <p class="error-msg">
                We could not locate the requested contract record in the active database.<br><br>
                It may have been moved to the archives.
            </p>
            <!-- NOTE: Adjust the href path (e.g., ../user_archives.php) if needed depending on your folder structure -->
            <a href="../../users/user_archives.php" class="btn-archive">
                <i class='bx bx-archive-in'></i> Check Archives
            </a>
        </div>
    </body>
    </html>
...
// We use the contract_id found in step 2 to get all related lots
...
// --- 6.5 FETCH CONTRACT HISTORY ---
$historyData = [];

// 1. Get the Original Creation Date from tbl_main (group_acc = NONE)
$sql_orig_date = "SELECT date_added FROM tbl_main WHERE contract_id = ? AND group_acc = 'NONE' LIMIT 1";
...
// 2. Fetch archived versions from BOTH tables
...
$stmt_renewed = $pdo->prepare($sql_renewed); // FIXED: Added the $ sign
...
if (!empty($dates)) {
    ...
    // A. LATEST EVENT (Newest Archive vs Current Main)
    $newestDate = $dates[0];
    ...
    $historyData[] = [
        'type' => $latestType,
        'source_table' => $newestRepRow['source_table'], // Pass table for rendering
        'display_date' => $newestDate,
        'user_id' => $sourceRow['user_id'],
        'from_data' => $newestRepRow,
        'to_data' => $sourceRow
    ];
    ...
    for ($i = 0; $i < count($dates) - 1; $i++) {
        ...
        // Determine Type based on the OLDER snapshot (the event that happened at this time)
        $eventHistoryType = ($olderRepRow['source_table'] == 'tbl_renewed_contracts') ? 'renewed' : 'updated';

        $historyData[] = [
            'type' => $eventHistoryType,
            'source_table' => $olderRepRow['source_table'], // Pass table for rendering
    ...
    $historyData[] = [
        'type' => 'added',
        'source_table' => $oldestRepRow['source_table'], // Should be whatever the first history was
        'display_date' => $original_creation_date,
        'user_id' => $oldestRepRow['user_id'],
        'data' => $oldestRepRow
    ];
} else {
    // --- CASE: No history yet (New Contract) ---
    $historyData[] = [
        'type' => 'added',
        'source_table' => 'tbl_main', // Default source
        'display_date' => $original_creation_date,
        'user_id' => $sourceRow['user_id'],
        'data' => $sourceRow
    ];
...
if ($sourceRow['contract_id']) {
    try {
        ...
        // This ensures we only show the photos from the most recent update (latest batch),
        // effectively hiding older redundant backups.
...
function renderCompactContractView($data, $pdo, $sourceTable = 'tbl_main', $doc_date = null)
{
    ...
    // --- RENDERING LOGIC (Same as before, but now using $lotsData) ---
    ob_start();
?>
    <div class="history-view">
        ...
        <?php foreach ($lotsData as $lot): ?>
            <div class="lot-card">
                ...
                <div class="lot-card-body" style="padding: 15px;">
                    ...
                    <!-- GROUPS (Now displays all groups correctly) -->
        ...
        <!-- 2. INCEPTION DATES (Moved to Bottom) -->
        ...
        // --- END DOCUMENTS LOGIC ---
        ?>
    </div> <!-- This closes the .history-view div -->
<?php
    return ob_get_clean();
}
?>
...
<head>
    ...
    <title>View Contract | Land Asset Management</title>
    <style>
        :root {
            --primary: #2E7D32;
        ...
        /* Renew Button (Blue - Indicates New Action) */
        .btn-renew {
        ...
        .history-content {
            ...
            /* REMOVED: transition: max-height 0.4s ease-out; */

            /* By removing max-height/overflow constraints, the section takes up its natural space 
       and allows the browser's main scrollbar to work. */
            display: none;
        ...
        .history-content.open {
            display: block;
            /* Simply show it when open */
            /* REMOVED: max-height: 5000px; */
            /* REMOVED: transition: max-height 0.6s ease-in; */
        }
        ...
        .comparison-grid {
            ...
            background: #fff;
            align-items: start;
            /* <--- ADD THIS: Aligns columns to the top instead of stretching them */
        }
        ...
        .comp-col-title {
            ...
            color: #333;
            /* Darker text for contrast on gray */
            padding: 10px;
            text-align: center;
            font-weight: 800;
            text-transform: uppercase;
        ...
        /* Specific overrides for history view */
        .history-view {
            /* Padding for inner content */
            padding: 0;
            background: transparent;
            /* Transparent so the white card background shows */
            display: flex;
            flex-direction: column;
            gap: 15px;
            /* REMOVED: height: 100%; */
        ...
        /* Clean up Lot Card styling for history */
        .history-view .lot-card {
            border: none;
            box-shadow: none;
            margin-bottom: 0;
            padding: 0 15px 15px 15px;
            /* Horizontal padding to match info-grid */
        }
        ...
        /* Ensure the Inception Dates box at the bottom has the same padding */
        ...
        .history-item-narrow {
            ...
            /* Center the card and keep bottom spacing */
        }
        ...
        .doc-preview-item {
            position: relative;
            /* REQUIRED: Anchors delete button to this card */
            display: inline-flex;
            /* REQUIRED: Allows card width to match image exactly (variable width) */
            flex-direction: column;
            /* REQUIRED: Stacks Image on top, Filename below */
            padding: 8px;
            /* Adds space inside the card so button doesn't overlap content */
            border: none;
...
<body class="sidebar-closed">
    ...
    <div class="view-container">
        ...
        <div class="view-header">
            <div class="header-info">
                <h1><i class='bx bx-file-find'></i> View Contract</h1>
        ...
        <div class="section-content">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value" style="color: var(--primary); font-weight:600;">Active</span>
                </div>
            </div>
                    ...
                    <div class="lot-card">
                        ...
                        <div class="lot-card-body">
                            ...
                            <div class="info-grid" style="margin-bottom: 30px;">
                                <div class="info-item">
                                    <span class="info-label">Lease Status</span>
                                    <span class="info-value" style="color: var(--primary); font-weight:600;"><?= e($lot['lease_status']) ?></span>
                                </div>
                                <div class="info-item">
                                    <span class="info-label">Contract Status</span>
                                    <span class="info-value"><?= e($lot['contract_status']) ?></span>
            ...
            <div class="info-grid" style="margin-bottom: 40px; background: var(--primary-light); padding: 20px; border-radius: 8px;">
                <div class="info-item">
                    <span class="info-label">Start Date</span>
                    <span class="info-value" style="color: var(--primary-dark); font-weight:600;"><?= e($old_start_date) ?></span>
                </div>
                <div class="info-item">
                    <span class="info-label">Expiry Date</span>
                    <span class="info-value" style="color: var(--primary-dark); font-weight:600;"><?= e($old_expiry_date) ?></span>
                </div>
                <div class="info-item">
                    <span class="info-label">Paid Up Date</span>
            ...
            <!-- 2. INCEPTION DATES (I moved this down slightly in logic, but keeping HTML order) -->
            <!-- ... (Your existing Inception Dates code stays here) ... -->
            ...
            <?php else: ?>
                <!-- Changed class to preview-grid to match new style -->
                <div class="preview-grid">
                    <?php foreach ($documents as $doc):
                        $ext = strtolower(pathinfo($doc['file_path'], PATHINFO_EXTENSION));
                        $is_image = in_array($ext, ['jpg', 'jpeg', 'png']);
                    ?>
                        <!-- Changed class to doc-preview-item to match new style -->
                        <a href="../documents/<?= $doc['file_path'] ?>" target="_blank" class="doc-preview-item">

                            <?php if ($is_image): ?>
                                <!-- IMAGE PREVIEW: Changed class to img-preview-box -->
                                <div class="img-preview-box">
                                    <img src="../documents/<?= $doc['file_path'] ?>" alt="<?= htmlspecialchars($doc['file_name']) ?>">
                                </div>
                            <?php else: ?>
                                <!-- FILE ICON PREVIEW: Changed class to img-preview-box -->
                                <div class="img-preview-box">
                                    <?php if ($ext == 'pdf'): ?>
                                        <i class='bx bxs-file-pdf' style="color:#d32f2f; font-size: 3rem;"></i>
                                    <?php elseif (in_array($ext, ['doc', 'docx'])): ?>
                            ...
                            <!-- Changed structure to match filename-info/filename-text -->
                            <div class="filename-info">
                                <div class="filename-text"><?= htmlspecialchars($doc['file_name']) ?></div>
                                <!-- REMOVED: doc-cat line to fix undefined index error -->
                                <!-- <span class="doc-cat">...</span> -->
                            </div>
                        </a>
                    <?php endforeach; ?>
                </div>
            ...
            <?php
            // === UPDATED LOGIC FOR BUTTON VISIBILITY ===

            $showRenew = false;
            $showUpdate = true; // Default: Update is visible

            // SCENARIO 1: Near Expiration
            if ($sourceRow['lease_status'] === 'Near Expiration') {
                $showRenew = true;
            }

            // SCENARIO 2: Expired
            // User Request: Show Renew and Remove, HIDE Update
            if ($sourceRow['lease_status'] === 'Expired') {
                $showRenew = true;  // Show Renew
                $showUpdate = false; // Hide Update
            }
            ?>

            <!-- ACTION BUTTONS (Updated Visibility Logic) -->
            <!-- Added 'justify-content: space-between' to separate Left (Google Earth) and Right (Action Buttons) -->
            <!-- <div class="actions-container" style="justify-content: space-between;"> -->
                <div class="actions-container" style="justify-content: end;">
              ...
              text-decoration: none; padding: 8px 18px; border-radius: 50px; font-weight: 600; font-size: 0.85rem; transition: all 0.2s; cursor: pointer;"
                ...
                <!-- RIGHT SIDE: Renew / Update / Archive Buttons -->
                <!-- Wrapper to keep these grouped together on the right -->
                <div style="display: flex; align-items: center; gap: 12px;">

                    <!-- 1. Renew Contract (Visible if Near Expiration OR Expired) -->
                    <?php if ($showRenew): ?>
                        <button onclick="openRenewModal()" class="btn-action btn-renew">
                            <i class='bx bx-refresh'></i> Renew Contract
                        </button>
                    <?php endif; ?>

                    <!-- 2. Update Contract (Visible ONLY if NOT Expired) -->
                    <?php if ($showUpdate): ?>
                        <button onclick="openUpdatePage()" class="btn-action btn-update">
                            <i class='bx bx-edit-alt'></i> Update Contract
                        </button>
                    <?php endif; ?>

                    <!-- 3. Archive Contract (Always Visible) -->
                    <button onclick="openDeleteModal()" class="btn-action btn-archive">
                        <i class='bx bx-archive'></i> Archive Contract
                    </button>
                </div>
    ...
    <div class="history-toggle-wrapper">
        ...
        <button id="btnHistoryToggle" class="btn-history-toggle" onclick="toggleHistory()">
            Show Contract History <i class='bx bx-chevron-down'></i>
        </button>
    </div>
        ...
        <div class="history-inner">
                    ...
                    <div class="history-item <?= ($item['type'] == 'added') ? 'history-item-narrow' : '' ?>">
                        ...
                        <div class="history-header">
                            <div class="history-header-top">
                                <!-- UPDATED BADGE LOGIC: Checks type for 'updated' or 'renewed' -->
                            ...
                            <div class="comparison-grid">
                                ...
                                <div class="comp-col">
                                    ...
                                    if ($item === $historyData[0]) {
                                        // Latest Event (e.g., May 5): FROM is state before update (May 4).
                                        // We check next item's display_date (May 4) to match backup logic.
                                        // If next item exists, use its date. Otherwise fallback to current from_data date.
                                        $nextItem = isset($historyData[$i + 1]) ? $historyData[$i + 1] : null;
                                        $fromDocDate = ($nextItem) ? $nextItem['display_date'] : $item['from_data']['date_added'];
                                    } else {
                                        // Previous Events (e.g., May 4): FROM is state before update (Apr 27).
                                        // We check next item's display_date (Apr 27) to match original creation logic.
                                        $nextItem = isset($historyData[$i + 1]) ? $historyData[$i + 1] : null;
                                        $fromDocDate = ($nextItem) ? $nextItem['display_date'] : $item['from_data']['date_added'];
                                    }
                                    ?>
                                ...
                                <div class="comp-col">
                                    ...
                                    if ($item === $historyData[0]) {
                                        ...
                                        $toTable = isset($item['to_data']['source_table']) ? $item['to_data']['source_table'] : 'tbl_main';
                                    }

                                    // Determine Doc Date for "TO" column
                                    // Use display_date (The Event Date) to match photos added DURING this update
                        ...
                        <?php else: ?>
                            ...
                            <div style="padding: 20px;">
                                <!-- Pass display_date (Original Creation Date) so it doesn't pick up future photos -->
            ...
            <?php else: ?>
                ...
                <div style="text-align: center; padding: 40px; color: #999; font-style: italic; font-size: 1rem;">
                    ...
                    No previous history for this contract.
                </div>
                <!-- END NEW MESSAGE -->
    ...
    <!-- MODAL (Outside Container, Inside Body) -->
    <div id="deleteModal" class="simple-modal-overlay">
        <div class="simple-modal-box">
            <i class='bx bx-error-circle' style="font-size: 50px; color: #d32f2f; margin-bottom: 15px;"></i>
            <h3>Confirm Archival</h3>
            <p>Are you sure you want to archive this contract record? This action cannot be undone.</p>

            <div class="modal-actions">
                <button class="btn-modal btn-modal-cancel" onclick="closeDeleteModal()">Cancel</button>
                <button class="btn-modal btn-modal-delete" onclick="confirmDelete()">Archive</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const currentContract = {
            ...
            lot: "<?= e($sourceRow['lot_no']) ?>", // Using first lot as anchor
            start: "<?= e($old_start_date) ?>",
            expiry: "<?= e($old_expiry_date) ?>",
            paid: "<?= e($old_paid_up_date) ?>"
        };

        // --- 3. RENEW CONTRACT LOGIC ---
        function openRenewModal() {
            // Option A: Redirect to Add Contract Page
            // We use window.open to ensure parent stays open
            const url = `renew_contract.php?contract_id=${encodeURIComponent(currentContract.contract_id)}`;
            window.open(url, '_blank'); // <--- FIXED: Changed 'addUrl' to 'url'

            // Note: If you want to pre-fill the add_contract.php with old data 
            // you would need to modify add_contract.php to read from localStorage.
            // For now, this just opens a fresh form.
        }

        // --- 1. OPEN UPDATE PAGE ---
        function openUpdatePage() {
            // Use contract_id instead of Party/Dates
            const url = `update_contract.php?contract_id=${encodeURIComponent(currentContract.contract_id)}`;

            window.open(url, '_blank'); // Opens in NEW TAB
        }

        // --- 2. DELETE MODAL FUNCTIONS ---
        ...
        function confirmDelete() {
            ...
            formData.append('delete_contract_block', '1'); // Trigger Block Delete Logic
            formData.append('contract_id', currentContract.contract_id); // <--- USE ID

            fetch('../actions/archive_contract_action.php', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert("Error: " + (data.message || "Could not archive record."));
                        // Reset button
        ...
        function toggleHistory() {
            ...
            if (content.classList.contains('open')) {
                btn.innerHTML = 'Hide Contract History <i class=\'bx bx-chevron-up\'></i>';
            } else {
                btn.innerHTML = 'Show Contract History <i class=\'bx bx-chevron-down\'></i>';
            }
        }
    </script>
</body>
...
<!-- FIXED Contract History STARTS HERE -->