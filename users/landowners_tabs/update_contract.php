...
// This ensures that even if dates are changed during the update, we still update the correct contract.
if ($old_contract_id) {
...
if ($old_contract_id) {
    try {
        ...
        // This ensures we only show the photos from the most recent update, hiding older redundant backups.
        ...
        // Bind the contract_id twice (once for the main query, once for the subquery)
...
<head>
    ...
    <title>Update Contract | Land Asset Management</title>
    <style>
...
<body class="sidebar-closed">
        ...
        <main>
            <div class="update-container">
                <div class="update-header">
                    <div>
                        <h2><i class='bx bx-edit-alt'></i> Update Contract</h2>
                        <p>
                ...
                <form id="updateContractForm">
                    ...
                    <button type="button" id="btnSaveChanges" onclick="validateAndConfirmUpdate()" class="btn btn-primary" style="width: 100%; padding: 15px; justify-content: center; font-size: 1.1rem;" disabled>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            const headerInputs = document.querySelectorAll('#updateContractForm input:not([type="hidden"]), #updateContractForm select');
        ...
        function createLotBlockHTML(index, data = {}) {
            ...
            const updateTitleFunc = `updateLotTitle(this, ${index})`;
            ...
            div.innerHTML = `
                ...
                <div class="form-grid">
                    <div class="form-group">
                        <label>Lot No</label>
                        <input type="text" name="lots[${index}][lot_no]" id="lot_no_${index}" value="${data.lot_no || ''}" required oninput="${updateTitleFunc}" style="font-weight:bold;">
                    </div>
                    <div class="form-group">
        ...
        // --- FEATURE: Live Update Title ---
        function updateLotTitle(input, index) {
        ...
        window.confirmDeleteLot = function() {
            ...
            if (mainIdToDelete) {
                ...
                fetch('../actions/update_contract_action.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.json())
        ...
        window.validateAndConfirmUpdate = function() {
            ...
            if (confirm("Are you sure you want to update this contract?")) {
                submitUpdateForm();
            }
        }
        ...
        window.submitUpdateForm = function() {
            ...
            formData.append('update_contract', '1');
            ...
            // We need to make it globally accessible or update the hidden input inside the IIFE.

            // SIMPLEST FIX: Update the hidden input whenever docsToDelete changes.
            ...
            fetch('../actions/update_contract_action.php', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        const newUrl = `update_contract.php?contract_id=${encodeURIComponent(contractId)}`;
                        // --- FIX END ---
                        ...
                        if (wasDeleted) {
                            ...
                            if (msgText) msgText.textContent = data.message || "Contract updated successfully.";
                            if (successModal) successModal.classList.add('show-modal');
            ...
            function renderExistingDocs() {
                ...
                existingDocsData.forEach((doc) => {
                    ...
                    if (isImage) {
                        ...
                        // Assuming update_contract.php is in pages/ and uploads is in root.
